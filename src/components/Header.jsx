    ...
    <header className="fixed top-0 left-0 z-50 w-full py-10">
      <div className="container flex h-14 items-center max-lg:px-5">
        <a href="" className="lg:hidden flex-1 cursor-pointer z-2">
          <img src="/src/assets/xora.svg" width={115} height={55} alt="xora" />
        </a>
        <div
          className={clsx(
        ...
        >
          {isOpen ? (
            <IoClose size={25} className="text-s4" />
          ) : (
            <img src="/src/assets/magic.svg" />
          )}
        </button>
      </div>
    </header>
...
const NavLink = ({ title }) => (
  <LinkScroll className="uppercase base-bold text-p4 transition-colors duration-500 cursor-pointer hover:text-p1 max-lg:my-4 max-lg:h-5">
    {title}
  </LinkScroll>
);
